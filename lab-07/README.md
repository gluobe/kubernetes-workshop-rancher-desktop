# Lab 07 - Jobs / Cronjobs

## Jobs

A Job creates one or more Pods and ensures that a specified number of them 
successfully terminate. As pods successfully complete, the Job tracks the 
successful completions. When a specified number of successful completions is 
reached, the task (ie, Job) is complete. Deleting a Job will clean up the Pods 
it created.

A simple case is to create one Job object in order to reliably run one Pod to 
completion. The Job object will start a new Pod if the first Pod fails or is 
deleted (for example due to a node hardware failure or a node reboot).

You can also use a Job to run multiple Pods in parallel.

## Task 0: Creating a namespace

Create a namespace for this lab:

```
kubectl create ns lab-07

---

namespace/lab-07 created
```

## Task 1: Running an example Job

Here is an example Job config. It computes π to 2000 places and prints it out. 
It takes around 10s to complete.

lab-07-job.yml:

```
apiVersion: batch/v1
kind: Job
metadata:
  name: pi
spec:
  template:
    spec:
      containers:
      - name: pi
        image: perl
        command: ["perl",  "-Mbignum=bpi", "-wle", "print bpi(500)"]
      restartPolicy: Never
  backoffLimit: 4
```

Now let’s run `kubectl create -f lab-07-job.yml -n lab-07` to deploy the example.

## Task 2: Check the status of the job

```
kubectl describe jobs/pi -n lab-07

---

Name:             pi
Namespace:        lab-07
Selector:         controller-uid=f7bfc6f2-0819-44c6-a2f2-cb25a934d45e
Labels:           controller-uid=f7bfc6f2-0819-44c6-a2f2-cb25a934d45e
                  job-name=pi
Annotations:      batch.kubernetes.io/job-tracking: 
Parallelism:      1
Completions:      1
Completion Mode:  NonIndexed
Start Time:       Thu, 24 Feb 2022 08:52:14 +0100
Completed At:     Thu, 24 Feb 2022 08:55:22 +0100
Duration:         3m8s
Pods Statuses:    0 Running / 1 Succeeded / 0 Failed
Pod Template:
  Labels:  controller-uid=f7bfc6f2-0819-44c6-a2f2-cb25a934d45e
           job-name=pi
  Containers:
   pi:
    Image:      perl
    Port:       <none>
    Host Port:  <none>
    Command:
      perl
      -Mbignum=bpi
      -wle
      print bpi(500)
    Environment:  <none>
    Mounts:       <none>
  Volumes:        <none>
Events:
  Type    Reason            Age                    From            Message
  ----    ------            ----                   ----            -------
  Normal  SuccessfulCreate  5m31s                  job-controller  Created pod: pi--1-rvz94
  Normal  Completed         2m23s (x3 over 2m23s)  job-controller  Job completed
```

To view completed Pods of a Job, use `kubectl get pods -n lab-07`.

To list all the Pods that belong to a Job in a machine readable form, you can use a command like this:

```
pods=$(kubectl get pods -n lab-07 --selector=job-name=pi --output=jsonpath='{.items[*].metadata.name}')
echo $pods

kubectl logs $pods -n lab-07

3.
```

### Job Termination and Cleanup

When a Job completes, no more Pods are created, but the Pods are not deleted 
either. Keeping them around allows you to still view the logs of completed pods 
to check for errors, warnings, or other diagnostic output. The job object also 
remains after it is completed so that you can view its status. It is up to the 
user to delete old jobs after noting their status. Delete the job with kubectl 
(e.g. kubectl delete jobs/pi or kubectl delete -f ./lab-07-job.yml). When you delete 
the job using kubectl, all the pods it created are deleted too.

## Cronjob

A Cron Job creates Jobs on a time-based schedule.

One CronJob object is like one line of a crontab (cron table) file. It runs a 
job periodically on a given schedule, written in Cron format.

## Task 3: Creating a Cron Job

Cron jobs require a config file. This example cron job config .spec file prints 
the current time and a hello message every minute:

Check out the following CronJob yaml:

```
apiVersion: batch/v1
kind: CronJob
metadata:
  name: hello
spec:
  schedule: "* * * * *"
  jobTemplate:
    spec:
      template:
        spec:
          containers:
          - name: hello
            image: busybox
            imagePullPolicy: IfNotPresent
            command:
            - /bin/sh
            - -c
            - date; echo Hello from the Kubernetes cluster
          restartPolicy: OnFailure
```

Run the example CronJob by using this command:

```
kubectl create -f https://k8s.io/examples/application/job/cronjob.yaml -n lab-07
```

> NOTE: above we used an URL as the input for `kubectl create`

After creating the cron job, get its status using this command:

```
kubectl get cronjob hello -n lab-07
```

As you can see from the results of the command, the cron job has not scheduled 
or run any jobs yet. Watch for the job to be created in around one minute:

```
kubectl get jobs --watch -n lab-07
```

The output is similar to this:

```
NAME             COMPLETIONS   DURATION   AGE
hello-27428160   1/1           9s         62s
hello-27428161   1/1           1s         2s
```

Delete the Cron Job

```
kubectl delete cronjob hello -n lab-07
```

## Task 4: Cleaning up

In order to delete our daemonset use the following command:

```
kubectl delete ns lab-07

---

namespace "lab-07" deleted
```